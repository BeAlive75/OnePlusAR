 ...
 */
package com.oneplusar.android.plugin.googlemap;
...
public class GoogleMapWorldPlugin implements WorldPlugin, BitmapCache.OnExternalBitmapLoadedCacheListener {
	 ...
	 */
	protected void addGooGleMapPlugin(OnePlusARObject oneplusarObject) {
		if (oneplusarObject instanceof GeoObject) {
			if (!oneplusarObject.containsAnyPlugin(GoogleMapGeoObjectPlugin.class)) {
				GoogleMapGeoObjectPlugin plugin = new GoogleMapGeoObjectPlugin(this, oneplusarObject);
				oneplusarObject.addPlugin(plugin);
				createMarker((GeoObject) oneplusarObject, plugin);
			}
		}
	}
	...
	public void createMarkers() {
		...
		for (int i = 0; i < mWorld.getOnePlusARObjectLists().size(); i++) {
			OnePlusARObjectList list = mWorld.getOnePlusARObjectList(i);
			for (int j = 0; j < list.size(); j++) {
				OnePlusARObject oneplusarObject = list.get(j);
				if (oneplusarObject instanceof GeoObject) {
					createMarker((GeoObject) oneplusarObject);
				}
			}
		}
	}
	...
	private void addPluginToAllObjects() {
		...
		for (OnePlusARObjectList list : OnePlusARLists) {
			for (OnePlusARObject oneplusarObject : list) {
				addGooGleMapPlugin(oneplusarObject);
			}
		}
	}
	...
	public void onOnePlusARObjectRemoved(OnePlusARObject oneplusarObject, OnePlusARObjectList oneplusarObjectList) {
		if (oneplusarObject instanceof GeoObject) {
			GeoObject geoObject = (GeoObject) oneplusarObject;
	...
	/**
	 * Retrieve the {@link com.oneplusar.android.world.GeoObject GeoObject} that
	 * owns an specific {@link Marker}
	 * 
	 * @param marker
	 *            The Marker that whant's to be checked
	 * @return The {@link com.oneplusar.android.world.GeoObject GeoObject} owner
