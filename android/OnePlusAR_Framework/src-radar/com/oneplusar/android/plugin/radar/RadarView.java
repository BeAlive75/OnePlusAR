...
public class RadarView extends ImageView {
	...
	private void drawRadarPoints(Canvas canvas) {
		...
		for (int i = 0; i < mRadarPlugin.getWorld().getOnePlusARObjectLists().size(); i++) {
			OnePlusARObjectList list = mRadarPlugin.getWorld().getOnePlusARObjectList(i);
			for (int j = 0; j < list.size(); j++) {
				OnePlusARObject oneplusarObject = list.get(j);
				RadarPointPlugin radarPointPlugin = (RadarPointPlugin) oneplusarObject
