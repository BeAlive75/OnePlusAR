...
public class SimpleCameraWithMaxFarMinAwayActivity extends FragmentActivity implements OnSeekBarChangeListener {
	...
	public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
		if (seekBar == mSeekBarPullCloserDistance) {
			mOnePlusARFragment.setPullCloserDistance(progress);
		} else if (seekBar == mSeekBarPushAwayDistance) {
			mOnePlusARFragment.setPushAwayDistance(progress);
		} else if (seekBar == mSeekBarMaxDistanceToRender) {
			mOnePlusARFragment.setMaxDistanceToRender(progress);
		} else if (seekBar == mSeekBarDistanceFactor) {
			mOnePlusARFragment.setDistanceFactor(progress);
		}
		updateTextValues();
	}
	
	private void updateTextValues() {
		mTextValues.setText("dst factor=" + mOnePlusARFragment.getDistanceFactor() + " max dst render="
				+ mOnePlusARFragment.getMaxDistanceToRender() + "\npull closer="
				+ mOnePlusARFragment.getPullCloserDistance() + " push away="
				+ mOnePlusARFragment.getPushAwayDistance());
	}
